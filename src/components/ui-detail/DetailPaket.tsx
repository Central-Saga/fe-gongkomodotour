...
const formatPrice = (price: string): string => {
  const cleanPrice = price.replace(/,/g, "").replace(/[^\d.]/g, "");
  const numPrice = parseFloat(cleanPrice || "0");
  if (Number.isNaN(numPrice)) return price;
  return Math.floor(numPrice).toLocaleString("id-ID");
};
...
const DetailPaket: React.FC<DetailPaketProps> = ({ data, type }) => {
  ...
  const hasInfoTab =
    (() => {
      ...
      const guideFee1Num = data.flightInfo?.guideFee1 ? parseFloat(data.flightInfo.guideFee1) : 0;
      const guideFee2Num = data.flightInfo?.guideFee2 ? parseFloat(data.flightInfo.guideFee2) : 0;
      const hasFlight = (data.flightSchedules?.length ?? 0) > 0 || !!(guideFee1Num > 0 || guideFee2Num > 0);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-6">
                  ...
                  {(() => {
                    const guideFee1Num = data.flightInfo?.guideFee1 ? parseFloat(data.flightInfo.guideFee1) : 0;
                    const guideFee2Num = data.flightInfo?.guideFee2 ? parseFloat(data.flightInfo.guideFee2) : 0;
                    const hasFlightSchedules = (data.flightSchedules?.length ?? 0) > 0;
                    const hasGuideFees = guideFee1Num > 0 || guideFee2Num > 0;
                    return hasFlightSchedules || hasGuideFees;
                  })() && (
                    <div className="bg-[#f5f5f5] p-6 rounded-lg shadow-sm min-h-[250px] flex flex-col">
                        ...
                        {data.flightSchedules && data.flightSchedules.length > 0 ? (
                          ...
                          <div className="text-gray-600 space-y-2">
                            {type === "private" ? (
                              <>
                                {(() => {
                                  const gf1 = data.flightInfo?.guideFee1 ? parseFloat(data.flightInfo.guideFee1) : 0;
                                  const gf2 = data.flightInfo?.guideFee2 ? parseFloat(data.flightInfo.guideFee2) : 0;
                                  return (
                                    <>
                                      {gf1 > 0 && <p>Guide Fee (Per Day): IDR {formatPrice(data.flightInfo.guideFee1)}</p>}
                                      {gf2 > 0 && <p>Guide Fee (Per 5 pax): IDR {formatPrice(data.flightInfo.guideFee2)}</p>}
                                    </>
                                  );
                                })()}
                              </>
