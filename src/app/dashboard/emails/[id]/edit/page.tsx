...
export default function EditEmailPage({ params }: EditEmailPageProps) {
    ...
    const fetchEmail = async () => {
      try {
        ...
        form.reset({
          ...
          body: email.body,
          recipient_type: email.recipient_type as "all_customers" | "subscribers" | "spesific_list",
          status: email.status as "Draft" | "Scheduled" | "Sent" | "Failed",
          scheduled_at: email.scheduled_at || "",
        })

        // Set recipients if email has specific recipients
        if (email.recipient_type === "spesific_list" && email.recipients && email.recipients.length > 0) {
          setRecipients(email.recipients.map(r => r.recipient_email))
        }
  ...
  const onSubmit = async (values: z.infer<typeof emailSchema>) => {
    try {
      ...
      const data = {
        ...values,
        recipients: values.recipient_type === "spesific_list" ? recipients.map(recipientEmail => ({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container max-w-7xl mx-auto px-4 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Edit Email</h1>
          <p className="text-gray-500 mt-2">Perbarui informasi email</p>
        </div>
